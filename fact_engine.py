...
1. 将事实与证据逐一比对
2. **证据与稿件陈述冲突 → "错误"**
   - 即使证据没有直接否定，但关键要素（主体/时间/地点/级别/序号/批次）不一致即判错
   - 例如：稿件说"X被列入内蒙古自治区级非遗"，证据显示"X是山西省非遗" → 判错误
   - 例如：稿件说"2010年列入"，证据显示"2008年列入" → 判错误
   - 例如：稿件说"列入第一批名录"，证据中第一批名单里未出现该主体 → 判错误
   - 例如：稿件说"第X届/第X批/第X次"，证据显示的批次/届次/次数不同 → 判错误
   - 不要以"证据未直接提及本稿件的说法"为由回避判断
3. **"通过"的门槛要高**：证据必须**明确支持**事实的每一个关键要素
   - 仅仅证据的主题与事实相关不等于"支持"——必须在证据中找到该主体/该序号/该时间的明确对应
   - 例如：稿件说"X村列入第一批传统村落"，证据是关于第一批传统村落的文件但文中未出现X村 → 应判"未搜到"而非"通过"
